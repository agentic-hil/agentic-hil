...
| `changed` | the two digests differ — the file on disk is not the one this server loaded. That is the whole claim: nothing is said about what the file now contains | restart the MCP server. If it does not come back, the startup refusal names what is wrong with the file; repair it and restart again. `agentic-hil doctor` produces the same refusal without stopping anything | `true` |

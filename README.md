...
`%APPDATA%` and `%LOCALAPPDATA%` are standard Windows working folders and the path trust check accepts them. What it refuses is a path a **nameable account other than you, SYSTEM or Administrators** can write, delete or re-permission. An installed packaged application's app-capability ACE (`S-1-15-3-*`) on `%USERPROFILE%\AppData` is reported and not refused: that identity runs in an AppContainer with a subset of your own rights, and your ordinary unpackaged processes can already rewrite the file in full. A SID the local security authority answered about — saying nothing on this machine maps to it — is likewise named rather than refused on a machine in a workgroup, where its own SAM is the only account database that answer can come from; a stock `%LOCALAPPDATA%` carries one. On a domain member the same `ERROR_NONE_MAPPED` also means the domain did not answer, and a live account (or one carried only in SID history) is indistinguishable from an orphan at that point, so it refuses there and `windows_path_trust: permissive` is the documented answer. What is *not* tolerated is what the check could not establish at all: a principal lookup that failed for some other reason, or an ACL that could not be read. Those are ignorance rather than a clean bill of health, and a gate that accepts them is a gate that opens whenever the lookup behind it breaks.

Where a refusal does remain, the override is a supported binding, not a debug switch: put the configuration and `state_root` under `%USERPROFILE%\.agentic-hil` and select it with `AGENTIC_HIL_CONFIG`. A configuration reached that way is read exactly like a discovered one. The refusal names the holder in `untrusted_principals`. The top-level `windows_path_trust: standard | strict | permissive` key sets how far the check goes — `strict` is the rule as it stood before 0.8.0, `permissive` is the visible override for an environment whose ACLs do not describe its trust boundary. It governs that configuration's own `state_root` and what is derived from it, and nothing else: the configuration directory, the MCP server executable and `~/.agentic-hil/device-locks` stay on `standard`, because one project's file may not weaken the mutex another project's run depends on. Never relax an ACL to pass the check, and never redirect `%APPDATA%` or `%LOCALAPPDATA%` to silence it: that hides which policy was in force. Full rules and the measured per-directory verdicts: `agentic-hil://reference/platform-paths`.
...
| `changed` | the two digests differ and the file that is there now loads | restart the MCP server; it loads the file that exists now | `true` |
| `invalid` | the two digests differ and the file that is there now will not load — invalid YAML, rejected by the schema, or refused by one of the document checks the startup loader runs; `backend_error` names which | repair the file, *then* restart. A restart before that stops the server and does not bring one back | `true` |

...
`%APPDATA%` and `%LOCALAPPDATA%` are standard Windows working folders and the path trust check accepts them. What it refuses is a path a **nameable account other than you, SYSTEM or Administrators** can write, delete or re-permission. An installed packaged application's app-capability ACE (`S-1-15-3-*`) on `%USERPROFILE%\AppData` is reported and not refused: that identity runs in an AppContainer with a subset of your own rights, and your ordinary unpackaged processes can already rewrite the file in full. A SID nothing can resolve, and an ACL that could not be read at all, are likewise named rather than refused — the second is ignorance, not a verdict, and refusing it locked out every sandbox, service account and CI runner.

Where a refusal does remain, the override is a supported binding, not a debug switch: put the configuration and `state_root` under `%USERPROFILE%\.agentic-hil` and select it with `AGENTIC_HIL_CONFIG`. A configuration reached that way is read exactly like a discovered one. The refusal names the holder in `untrusted_principals`. The top-level `windows_path_trust: standard | strict | permissive` key sets how far the check goes — `strict` is the rule as it stood before 0.8.0, `permissive` is the visible override for an environment whose ACLs do not describe its trust boundary. Never relax an ACL to pass the check, and never redirect `%APPDATA%` or `%LOCALAPPDATA%` to silence it: that hides which policy was in force. Full rules and the measured per-directory verdicts: `agentic-hil://reference/platform-paths`.

...
Write a hardware test as a plan, not a script: one reviewable YAML file describes flash → open UART → break → dump, and an invalid plan is rejected in full before the first hardware action — no partially-validated run reaches the board. A step that fails at runtime stops the ones after it; the effects already committed stand, because a flash cannot be rolled back, and the report names the step that stopped the run. No leftover breakpoints, no orphaned debug or UART sessions — the same file behaves identically on your bench and in CI, and it diffs like code in a pull request.

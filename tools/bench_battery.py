...
* the configuration root and the state root are redirected into a temporary
  ...
  init`` here and no configuration the operator owns is read or replaced;
...
* HOME is deliberately left alone. The machine-wide device locks live under it,
  ...
  that isolated HOME would be a battery that cannot see a nightly job, or it
  this;
...
identity: what is recorded about hardware is that an identity was there, never
what it said.
"""
...
class Battery:
    ...
    def configure(self) -> str:
        ...
        self.config = Path(result["config_path"])
    ...
    def judge(self, check: Check, answered: subprocess.CompletedProcess[str], judged: dict, verdict: bool, decisive: str) -> Check:
        ...
        check.decisive_line = decisive
        return check

    def skip(self, check: Check, why: str) -> Check:
        check.outcome = SKIP
        check.decisive_line = why
        return check
    ...
    def check_probe_listing(self) -> Check:
        check = self.record(Check("debugger-probes", "probe discovery answers and says how far its count reaches"))
        ...
        # a fault is what this pins.
        ok = answered.returncode == 0 and result.get("ok") is True and result.get("complete") is False
        judged = {"probes": len(result.get("probes") or []), "complete": result.get("complete")}
    ...
    def check_plan_strict(self) -> Check:
        ...
        heading = answered.stdout.splitlines()[0] if answered.stdout.strip() else ""
    ...
    def check_project_plans(self) -> Check:
        ...
        # A plan is a document declaring steps. The project root can hold other
        # YAML, an example configuration among it, and handing one of those to
        # `check-plan` would report the project as broken over a file that was
        # never a plan.
        plans = sorted(
            path.name
            for path in self.project.glob("*.yaml")
            if not path.name.startswith("bench-battery-") and any(line.startswith("steps:") for line in path.read_text(encoding="utf-8", errors="replace").splitlines())
        )
        if not plans:
            return self.skip(check, "this project's root holds no plan to check")
    ...
    def check_doctor(self) -> Check:
        ...
        decisive = answered.stdout.splitlines()[0] if answered.stdout.strip() else answered.stderr[:200]
        return self.judge(check, answered, {"bound": ok}, ok, decisive)
    ...
    def check_probe_inventory(self) -> Check:
        ...
        with_a_serial = [probe for probe in probes if isinstance(probe.get("probe_id"), str) and probe["probe_id"].strip()]
    ...
    def check_reset(self) -> Check:
        ...
        carried = failure_worded_lines(printed) == list(warnings)
        ...
        judged = {"success_confirmed": step.get("success_confirmed"), "backend_warnings": len(warnings), "carried_verbatim": carried}
    ...
    def check_project_plan(self) -> Check:
        ...
        (self.project / "artifacts" / "reports").mkdir(parents=True, exist_ok=True)
        answered, report = self.document("test-reactor", "--test-config", plan)
        (self.project / "artifacts" / "reports" / "bench-battery.json").write_text(json.dumps(report), encoding="utf-8")
    ...
    def check_permission_refusal(self) -> Check:
        ...
        ok = (
            ...
            and f"{Path(self.product).name} grant {key}" in str(finding.get("next_step", ""))
        ...
        judged = {
            ...
            "grant_line_offered": f"grant {key}" in str(finding.get("next_step", "")),
    ...
    def check_run_evidence(self) -> Check:
        ...
        report = self.project / "artifacts" / "reports" / "bench-battery.json"
        ...
        answered = self.run("run-evidence", "--report", str(report.relative_to(self.project)), "--out", "artifacts/bench-battery-evidence")
        summary_path = self.project / "artifacts" / "bench-battery-evidence" / "job-summary.md"
        if answered.returncode != 0 or not summary_path.is_file():
            return self.judge(check, answered, {}, False, answered.stderr[:200] or answered.stdout[:200])
    ...
    def debugger_capture(self, log_path: object) -> str:
...
def main(argv: list[str] | None = None) -> int:
    ...
    Path(options.out).write_text(json.dumps(document, indent=2) + "\n", encoding="utf-8")

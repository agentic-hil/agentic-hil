/*
 * A board that answers, and answers wrongly.
 *
 * The demo in every respect a plan can see but one: the banner it prints once
 * at boot has an exclamation mark the demo's does not, and then the same LED
 * blinks. A plan that expects the demo's banner hears this board clearly and
 * has to call it a mismatch, which is the failure a firmware change produces
 * and the one a report has to tell apart from a board that never spoke.
 *

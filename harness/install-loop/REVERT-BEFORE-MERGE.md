...
- Per-agent USER-level MCP registration: `register_agent_mcp()` + `setup` wiring
  (`src/agentic_hil/cli.py`, tests) — Codex `~/.codex/config.toml`, opencode
  `~/.config/opencode/opencode.json`, Claude `~/.claude.json` (user scope). Plus the
  reframed "Configure MCP" doc + `docs/mcp-hosts.md` "Generated by setup" section.

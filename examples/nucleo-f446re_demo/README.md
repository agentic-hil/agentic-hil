...
The profile is read by the project half, so on a machine where `agentic-hil agent-install` has already run, `agentic-hil init` alone does the same discovery.

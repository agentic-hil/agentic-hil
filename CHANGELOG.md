...
- **Machine-read JSON goes out compact.** Every `tools/call` result is sent twice — once as `structuredContent`, once serialized into the content text block, which the MCP specification recommends so a host that does not read `structuredContent` still gets the result. That text block was indented for a reader it does not have: it is parsed, never read. It is now serialized with `separators=(",", ":")`, as are the `application/json` reference resources under `agentic-hil://reference/`, which `AGENTS.md` directs an agent to parse instead of reading the source. Measured over JSON-RPC against `mcp-stdio` on nine real tool calls and the JSON resources: 47325 → 43433 bytes, −8,2 % overall, −8,6 % across `tools/call` responses and −15,8 % across the text blocks alone. The text block is now byte-identical to `structuredContent`, which is the whole of what it was ever meant to be. Nothing a human opens changed: the `config-schema` resource is still served byte-for-byte from the shipped schema, and reports under `.agentic-hil/reports/`, debugger logs, audit sidecars, host configuration files and CLI output stay indented.

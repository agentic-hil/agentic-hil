...
- `agentic-hil setup --agent <agent>`: one-shot project setup (authoritative config + skill install + MCP registration + doctor) in a single command. It prepares a safe external `state_root`, validates the existing path chain without rewriting ancestor permissions, and fails closed with `unsafe_configured_path` when operator remediation is required.

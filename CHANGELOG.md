...
- **`setup` is split along its scope boundary and still works as one command.** It did four things in one transaction across two scopes with different lifetimes: install the agent skill and register MCP at user level (machine and agent scoped, once per machine), then write the authoritative config and run `doctor` (project scoped, once per project). One rollback set and one permission-smoothing pass spanned both. New `agentic-hil agent-install --agent <agent>` is the machine-wide half — skill, user-level MCP registration, and the check that a persistent trusted executable exists — with no workspace and no readable configuration involved. `agentic-hil init` is the project half: it now runs `doctor` after writing the config, takes an optional `--agent` for the agent write restriction, and keeps a config it finds instead of refusing, so both halves are idempotent. `agentic-hil setup` runs the two in that order, so a first run is still one command and a second project on the same machine needs `init` alone. Rollback stays inside its own half: a project step that fails cannot undo a machine-wide installation, or the reverse. Consequence on a stock Windows profile, where `%APPDATA%` fails the ancestor trust check: the configuration refusal no longer takes the skill and the MCP entry down with it. `setup` reports `ok: false` with `scopes.machine.ok` true, and the operator is left with a working agent and one location to fix instead of nothing. `--force` still reaches the machine-wide half only; rewriting an authoritative config remains `agentic-hil init --force`.

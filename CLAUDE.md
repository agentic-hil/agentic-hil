...
- Run the agent review loop in a container: `python tools/loop_in_container.py --task "..."` — needs Docker, and it is how the loop should be run. On Windows the reviewer's test runs die on NTFS permissions a previous round left, Codex cannot run the suite under its own sandbox, and the POSIX half is invisible; none of that happens on Linux. It mounts this repository read-write so the commits land here, and takes every other `tools/agent_review_loop.py` option unchanged.

...
10. For quarantined hardware, stop effects and read hardware_lease_status, then clear it with hardware_recover. A reason that names no hardware contact clears with no argument. A reason that needs somebody to look at the board needs operator_statement: ask the operator in chat what state the bench is in and pass their answer back verbatim — never invent one. The audit-broken family, and any case with nobody to ask, keep the operator's own route: relay the recover --confirm-safe-state --quarantine-id <id> line the refusal hands you and let them run it. If recovery returns config_changed, the operator verifies the config delta and reruns with --accept-config-change.

...
SERVER_INSTRUCTIONS = (
    ...
    "config_status.state and is not the same for all four: changed — ask the operator to restart the MCP server once, "
    "and it loads the file that exists now; invalid — the file that is there will not load, so a restart would stop "
    "this server and fail to bring one back: report backend_error, have the file repaired, and ask for the restart "
    "only after that; missing — the file is gone and there is nothing to restart onto, so it has to be restored "
    "first; unreadable — it is there and will not open, so it has to be made readable first. Never ask for a restart "
    "on invalid, missing or unreadable before the file has been repaired, restored or made readable. In every case, "
    "do not try to make the server pick the change up by editing the file again or by calling a configuration "
    "tool.\n"

...
SERVER_INSTRUCTIONS = (
    ...
    "config_stale: true says the file has changed since — the backend it names and the permissions it enforces are "
    "the ones loaded at startup, not the ones in the file now. Report it, ask the operator to restart the MCP server "
    "once, and do not try to make it pick the change up by editing the file again or by calling a configuration "
    "tool.\n"

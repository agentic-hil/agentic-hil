...
SERVER_INSTRUCTIONS = (
    ...
    "config_stale: true says the file is no longer the one it parsed — the backend it names and the permissions it "
    "enforces are the ones loaded at startup, not the ones in the file now. What to do about it is decided by "
    "config_status.state and is not the same for all three: `changed` — the file on disk differs from the one this "
    "server loaded; ask the operator to restart the MCP server once, and report the startup error if it does not come "
    "back, because the file it found is then the thing to repair; `missing` — the file is gone, so it has to be "
    ...
    "now contains, and it does not promise a restart will succeed. In every case, do not try to make the server pick "
    "the change up by editing the file again or by calling a configuration tool.\n"
    ...
    "this machine, and every permission in it is true — flashing, reset, raw debugger commands, mass "
    "erase, COM and CAN writes, artifact upload, unrestricted symbol access, and all three "
    "permissions.allow_config_* grants. The bench is workable from that file without anybody editing "
    "YAML. Report where it is and what it granted, name allow_mass_erase in particular because a mass "
    "erase cannot be taken back, and ask the operator which permissions this bench should not have.\n"

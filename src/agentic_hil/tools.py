...
# was authored by the agent: the hardware facts come from discovery and every
# permission below comes from the fixed skeleton, which sets all of them true.
# The bench is workable from this file as it stands, including flashing,
# resetting, raw debugger commands and mass erase.
#
# You are the one who narrows it. Tell the agent in prose which permission a bench
# should not have and it writes false here, with a note in `provenance`. It
# cannot write true into any of them, so nothing it does to this file can widen
# it — closing permissions.allow_config_permissions_write is the last permission
# change an agent can make at all. `agentic-hil init --force` regenerates this
# file with everything open again.
#
...
# flash_firmware on that probe is refused. Both are true here. Take away whichever
# of the two this bench does not need — or ask the agent to — and flashing works.
"""
...
def _generated_next_steps(config: AgenticHILConfig, *, created: bool) -> list[str]:
    steps = [
        "Report where this configuration is, that an agent generated it, and that every permission in it is granted — "
        "including allow_mass_erase, which cannot be taken back once it has run.",
        f"Ask the operator which of them this bench should not have. You can write false into any permission in "
        f"{config.config_path} with `project_config_set`; you cannot write true into one, so nothing you do here widens it.",
    ]
    if created:
        steps.append(

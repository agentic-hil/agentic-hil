...
# permissions come from the fixed skeleton or from the file this one replaced.
"""
GENERATED_CONFIG_HEADER_CLOSING = """#
# You are the one who narrows it. Tell the agent in prose which permission a bench
# should not have and it writes false here, with a note in `provenance`. Over MCP
# it cannot write true into any of them, so nothing it does to this file widens it
# — closing permissions.allow_config_permissions_write is the last permission
# change it can make there at all. Regenerating is yours, not its:
...
def _generated_config_header(document: JsonObject) -> str:
    ...
    if not narrowed:
        middle = (
            "# Every permission below is true. The bench is workable from this file as it\n"
            "# stands, including flashing, resetting, raw debugger commands and mass erase.\n"
        )
    else:
        listed = "\n".join(f"#   {path}" for path in narrowed)
        middle = (
            f"# {len(narrowed)} permission(s) below are false, carried over from the configuration\n"
            "# this one replaced rather than reset to the skeleton's open default:\n"
            f"{listed}\n"
...
def _project_config_create(
    ...
    *,
    ...
    with secure_user_file_lock(target_path):
        ...
        # puts back exactly the permissions already on disk, so a bench somebody
    ...
    result = {
        ...
        # carries the permissions of the file it replaced onto the entries that
        # were already in it and gives anything newly discovered the skeleton's
        # open default, so "every permission is granted" and "every permission was
        # carried over unchanged" are each true only sometimes, and an operator
        # cannot tell which from a sentence that never varies.
...
def _generated_summary(*, created: bool, narrowed: list[str]) -> str:
    if created:
        return (
            "Project configuration generated from attached hardware with every permission granted; this server can flash, "
            "reset and erase the bench from it, and can narrow any of those on the operator's request."
        )
    if not narrowed:
        return "Project configuration regenerated from attached hardware; every permission in it is granted."
    return (
        f"Project configuration regenerated from attached hardware; {len(narrowed)} permission(s) were carried over from "
        "the previous file as false and every other permission in it is granted. `narrowed_permissions` names them; "
        "anything this regeneration discovered for the first time is not among them and starts open."
    )
...
def _discover_for_generation(current: AgenticHILConfig | None, coordinator: HardwareCoordinator | None) -> tuple[JsonObject, JsonObject | None]:
    """Read what is attached, holding what a probe read holds.

    The same three things `project_config_adopt_hardware` does for the same two
    commands: the audit trail is proven writable before the board is touched, the
    host-wide enumeration pseudo-resource and the probe the enumeration selects
    are locked, and a raised exception leaves the locks taken and quarantined
    ...
    try:
        ...
        return {}, {**audit_unavailable(PROJECT_CONFIG_CREATE, error), "side_effect_committed": False, "side_effect_status": "not_started", "hardware_state": "unchanged", "retry_safe": False}
    if coordinator.blocked:
        return {}, {
            "ok": False,
            "tool": PROJECT_CONFIG_CREATE,
            "error_type": "resource_quarantined",
            "summary": "Hardware effects are blocked until unresolved cleanup or audit state is recovered, so the attached probe was not read and nothing was written.",
            "cleanup_required": True,
            "quarantined": True,
            "retry_safe": False,
    ...
    try:
        held.append(coordinator.acquire(DEBUGGER_DISCOVERY_RESOURCE))
    except CoordinationError as error:
        return {}, _generation_coordination_refusal(error, [DEBUGGER_DISCOVERY_RESOURCE])
    ...
    def before_connect(selected: str) -> JsonObject | None:
        ...
        try:
            ...
            return _generation_coordination_refusal(error, [resource])
        return None
    ...
    try:
        ...
        for lease in held:
            lease.quarantine("config_create_discovery_exception", error)
        raise
...
def _generation_coordination_refusal(error: CoordinationError, resources: list[str]) -> JsonObject:
...
def _generated_next_steps(config: AgenticHILConfig, *, created: bool, narrowed: list[str]) -> list[str]:
    granted = (
        ...
        if not narrowed
        else f"that every permission in it is granted except the {len(narrowed)} listed in `narrowed_permissions`, which were carried over from the file it replaced"
    )
    steps = [

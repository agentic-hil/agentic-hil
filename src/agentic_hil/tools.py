...
def project_config_create(workspace: Path, existing: AgenticHILConfig | None) -> JsonObject:
    """Generate this workspace's authoritative configuration, once.

    ``existing`` is the configuration this server is bound to, or None when it
    started without one. It is never taken from a caller's argument: creation is
    for the workspace the server is bound to and for no other, and no parameter
    exists that could name a different one."""
    try:
...
def _project_config_create(workspace: Path, existing: AgenticHILConfig | None) -> JsonObject:
    ...
    with secure_user_file_lock(target_path):
        record = agent_provisioning_record(workspace)
        if existing is None and record is not None:
            return _provisioning_spent(workspace, target_path, record)
        if existing is not None and not existing.permissions.allow_config_write:
            return _config_write_denied(existing, target_path)
        # Read-only, and the values it returns are the only ones the agent side
        # contributes. A failure refuses before the grant is spent: a file
        # generated without discovery would name no probe and no port, and
        # spending the one-time grant on it would leave the project worse off
        # than the config_file_not_found it started from.
        discovery = discover_attached_hardware()
        if not overall_success(discovery):
            return {
                **discovery,
                "tool": PROJECT_CONFIG_CREATE,
                "summary": f"{discovery.get('summary', 'Hardware discovery failed.')} No configuration was generated and the one-time provisioning grant is unspent.",
                "workspace_root": str(workspace),
                "next_step": "Attach exactly one supported probe and target, then call this tool again. Nothing was written and nothing was spent.",
            }
...
def _provisioning_spent(workspace: Path, target_path: Path, record: JsonObject) -> JsonObject:
    result = tool_error(
        PROJECT_CONFIG_CREATE,
        "permission_denied",
        "This workspace has already used its one-time agent provisioning and no configuration is at the expected path. "
        "Generating one is not open again: the grant is recorded outside the configuration precisely so that removing or "
        "moving the file does not return it.",
    )
    return {
        **result,
        "permission": "allow_config_write",
        "reason": "one_time_provisioning_consumed",
        "workspace_root": str(workspace),
        "path": str(target_path),
        "provisioning_record": record.get("path"),
        "granted_at": record.get("granted_at"),
        "next_step": (
            f"{result['next_step']} A person restores this project by running `agentic-hil init` from {workspace}, "
            "which needs no grant. Never delete the provisioning record."
        ),
        "side_effect_committed": False,
        "side_effect_status": "not_started",
        "hardware_state": "unchanged",
...
def _config_write_denied(existing: AgenticHILConfig, target_path: Path) -> JsonObject:
    result = tool_error(
        PROJECT_CONFIG_CREATE,
        "permission_denied",
        "Writing this project's configuration is denied by permissions.allow_config_write. A configuration already "
        "exists for this workspace, and this server may read it but not change it.",
    )
    return {
        **result,
        "permission": "allow_config_write",

...
class AgenticHILToolService:
    ...
    def _attempt_machine_recovery(self) -> JsonObject | None:
        """Clear a fully retryable incident without an operator, or refuse to.

        The verification is deliberately read-only. Leftover debugger processes
        of this owner are reaped host-side, then the probe is re-read through
        probe_target, which connects without resetting on every backend
        (OpenOCD `init; targets`, pyOCD `status`, ST-Link `-HOTPLUG`). Nothing
        here drives the target, so the physical state an operator would have to
        attest to cannot change underneath the check. Whatever a read-only
        re-read cannot settle keeps needing that operator.

        Returns the recovery report, or None when the incident is not machine
        recoverable or verification did not confirm the safe state."""
        reason = self.coordinator.retryable_incident()
        ...
        report = write_report(
            self.config,
            {
                ...
                "summary": "Retryable cleanup state was verified by a read-only probe re-read and cleared without operator attestation.",
        ...
        if report.get("audit_ok") is False:
            ...
            return None
        if not self.coordinator.resolve_retryable_incident(reason):
            return None

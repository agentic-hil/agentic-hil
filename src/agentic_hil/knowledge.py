...
- `agentic-hil init --agent <agent>` writes the location of a configuration bound this way into `external-projects.json` beside the projects directory, and it holds nothing but locations. Beside whichever of the two projects roots this profile can actually write, the platform default first and `~/.agentic-hil` after it, so a host whose profile directories are virtualized records its projects instead of refusing to set them up. A record that already exists is this user's record wherever it is, which is what keeps one profile from carrying two. The write refusals that run leaves in the agent's own settings are refreshed on every later run, and a project whose configuration the projects directory does not hold would otherwise be read there as a bench that is gone, so its rule would be taken back while the bench still wanted it. A recorded configuration that cannot be read leaves that question open and no rule is taken back at all; `agentic-hil uninstall` takes the record back together with the rules it explains.

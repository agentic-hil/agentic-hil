...
A configuration produced by `agentic-hil init` or `project_config_create` grants **every** permission it declares: flashing, reset, serial and CAN writes, artifact upload, unrestricted symbol access, and all three project-scoped `permissions.allow_config_*` grants. That is the owner's decision, taken after the destructive ones were named: the person who runs these benches owns them, and the previous closed default cost a working day of hand-edited YAML while protecting against nothing an agent with a shell was actually stopped by.

Two permissions are the exception and are written **`false`**: `allow_raw_debugger_commands` and `allow_mass_erase`. Validated flashing and unrestricted debugger access are mutually exclusive policies, so while either of those is true, `flash_firmware` on that probe is refused — a generated configuration that granted them was a bench that could not flash at all, which is what was reported. They are the exception rather than the interlock being weakened because neither of them grants anything: there is no MCP tool for raw debugger commands and none for mass erase, so every read of either is a deny site and turning one on subtracts flashing while adding no capability. Allow-by-default is for permissions that can do something. Setting one true remains an operator's edit, for a bench where something outside these tools drives the probe that way, and it costs flashing through them while it stands.

Review the file `setup` prints and take back what your bench should not have.
...
- An agent may set any permission in the file to `false` through it, gated by `permissions.allow_config_permissions_write` and recorded in `provenance`. That grant covers every permission key, not only the ones inside a `permissions:` block: the two that sit directly on a section, `artifacts.allow_upload` and `debug.allow_all_symbols`, are gated by it as well.
...
- **Regeneration is creation, not a permissions write, and it is not bound by it.** `project_config_create` on a workspace whose configuration is gone writes the skeleton at its defaults, and so does `agentic-hil init --force` at the command line. A regeneration over a configuration that is still there carries over the permissions of the configuration **the writing server loaded at startup** for the entries that are still in it — not a fresh read of the file, because a server does not reload and must not generate against a policy it is not enforcing. So in one running session a `project_config_set` narrowing followed by a `project_config_create` puts the loaded grant back into the file, and anything the regeneration discovers for the first time arrives at the skeleton's defaults. Regenerating is the operator's call and is treated as theirs; it is gated by `allow_config_write`, which is what an operator sets to `false` on a bench that is not to be regenerated from the agent side at all.
- **Anything a person does at the command line is theirs.** The `false`-only rule is not applied to CLI paths.

So an agent can only ever reduce its own authority *through the tool it has*. Widening is a person's, at the command line, and `agentic-hil init --force` is the command — it regenerates the configuration from attached hardware at the skeleton's defaults again, which grant everything but the two flashing is interlocked against. Treat a report that a permission is missing as a request for that command, not as something to work around.

The file-level deny rules `setup` writes into an agent host are a different mechanism and are unaffected by any of this.

`setup --agent claude-code` writes one deny rule per protected tree into that host's user settings, which is a lock on the front door and not a wall — a shell writes those files regardless. **For opencode, Agentic HIL writes no such rule, deliberately.** That host puts the decision in the operator's hands at the moment of the prompt: answering one permission prompt with "always" adds a session rule that allows every subsequent edit, and it outranks anything the configuration says. A rule written by setup would therefore be a lock a single click removes, and presenting it as protection would say something about this bench that is not true. Whether opencode's file tools may reach the authoritative config and `state_root` is set by the operator in `~/.config/opencode/opencode.json`, under `permission.edit`; `setup` reports that it wrote nothing, and removes the ineffective patterns earlier releases left there. Codex needs no rule: it sandboxes model-generated shell commands and leaves MCP servers outside that sandbox. `agentic-hil init --agent <agent>` writes those rules on a configuration that is already there too, without touching that file, so a bench whose first run was split (a plain `init`, then `agent-install`) and a bench whose configuration predates the rules both have a repair route that is not `init --force`. Every such run refreshes rather than only adds: a rule this tool wrote for a tree no configuration of this user's still names, a `state_root` an operator moved away from or a project that was removed, is taken back, because what it denies is writing to a tree the bench has left. Only patterns pointing inside the per-user roots Agentic HIL creates for itself are ever recognised as its own, so an operator's own deny rules come back exactly as they were written, and so do the rules protecting every other project on the machine.

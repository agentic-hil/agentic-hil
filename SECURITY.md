...
A configuration produced by `agentic-hil init` or `project_config_create` grants **every** permission it declares: flashing, reset, serial and CAN writes, artifact upload, unrestricted symbol access, and all three project-scoped `permissions.allow_config_*` grants. That is the owner's decision (hardci-hq#96), taken after the destructive ones were named: the person who runs these benches owns them, and the previous closed default cost a working day of hand-edited YAML while protecting against nothing an agent with a shell was actually stopped by.

Two permissions are the exception and are written **`false`**: `allow_raw_debugger_commands` and `allow_mass_erase`. Validated flashing and unrestricted debugger access are mutually exclusive policies, so while either of those is true, `flash_firmware` on that probe is refused — a generated configuration that granted them was a bench that could not flash at all, which is what hardci-hq#107 reported. They are the exception rather than the interlock being weakened because neither of them grants anything: there is no MCP tool for raw debugger commands and none for mass erase, so every read of either is a deny site and turning one on subtracts flashing while adding no capability. Allow-by-default is for permissions that can do something. Setting one true remains an operator's edit, for a bench where something outside these tools drives the probe that way, and it costs flashing through them while it stands.

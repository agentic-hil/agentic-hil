...
| `ok: false`, `error_type: installation_broken` | The manager stopped part way and the package did not survive it: `agentic-hil` still starts and dies with a `ModuleNotFoundError` | Run the line in `reinstall_command`, with the host stopped. It carries the extras and the scope this installation had. The install line from section 1 is the same repair without the extras spelled by hand |

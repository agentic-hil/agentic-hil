...
| `ok: true`, `already_current: true` | Nothing newer to install, both version fields are the same number, and `newest_release` is the index's own answer, asked before the claim was made | Nothing, unless `restart_required` is true. It is when a server started out of this installation is still up: nothing was replaced here, so that process answers with whatever was on disk when it started, and only its restart makes that this release |
...
| `ok: false`, `error_type: upgrade_blocked_by_recorded_option` | The manager had nothing to install while the index publishes something newer, and `held_back_by` names what this installation's own receipt records: an `exclude-newer`, or an exact requirement the manager did not name in words. No `uv tool` command clears either | Run the line in `reinstall_command`. It records the installation again without the option and keeps the extras, the `--with` packages and the interpreter it has |
...
Only the pinned case, the recorded-option case and the two failures are refusals, and only they exit non-zero: a machine that is already at the newest release has nothing wrong with it, so a provisioning script may run `agentic-hil upgrade` unconditionally. Read `restart_required` rather than assuming one: it is true when a server started out of this installation is still running or an agent host has this server registered, and false on a machine with neither, whatever moved on disk.

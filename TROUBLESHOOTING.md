...
| `ok: true`, `already_current: true` | Nothing newer to install, both version fields are the same number, and `newest_release` is the index's own answer, asked before the claim was made | Nothing. Do not restart: the same release would be reloaded |
...
Only the pinned case, the recorded-option case and the two failures are refusals, and only they exit non-zero: a machine that is already at the newest release has nothing wrong with it, so a provisioning script may run `agentic-hil upgrade` unconditionally. Read `restart_required` rather than assuming one: it is true on exactly the outcome that replaced something.

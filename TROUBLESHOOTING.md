...
```bash
python -m pip install --user --upgrade "agentic-hil>=0.7.1"
agentic-hil --version
...
```bash
uvx --from "agentic-hil>=0.7.1" agentic-hil --version                           # transient diagnostic only
uvx --from git+https://github.com/agentic-hil/agentic-hil@v0.7.1 agentic-hil --version # transient repository check
uv tool install --upgrade "agentic-hil>=0.7.1"                                  # persistent user-local install
```

`pipx run --spec "agentic-hil>=0.7.1" agentic-hil --version` is also only a transient diagnostic; use `pipx install "agentic-hil>=0.7.1"` for a persistent installation. If `agentic-hil` is installed but not found, add the user-level executable directory to `PATH` with `uv tool update-shell` or `pipx ensurepath` and open a fresh shell. If neither `uv` nor `pipx` exists, install `uv` user-locally first (`curl -LsSf https://astral.sh/uv/install.sh | sh`). Never use `sudo pip` or `pip install --break-system-packages`.
...
If the refusal is `unsafe_configured_path` on the location rather than the file contents, move the configuration as `agentic-hil://reference/platform-paths` describes and point `AGENTIC_HIL_CONFIG` at the new absolute path. Never relax an ACL or take ownership to pass the check.

`%APPDATA%` and `%LOCALAPPDATA%` are usually not this case from 0.8.0 on. They are standard Windows working folders, and on a machine in a workgroup they pass — including on a profile where an installed packaged application holds an app-capability ACE on `AppData`: the grant is real, but it belongs to an AppContainer identity holding a subset of your own rights, and your ordinary unpackaged processes can rewrite the file anyway. What is still refused is a **nameable account other than you, SYSTEM or Administrators** holding write, delete or WRITE_DAC on the path or an ancestor. A stock `%LOCALAPPDATA%` also carries an `S-1-5-21-*` SID the security authority answers about by saying nothing here maps to it, and that is reported rather than refused.

**The join state is part of that, not a footnote to it.** "Nothing here maps to it" is only a clean bill where your own SAM was the only thing asked. On a domain member, or on a machine joined to an Entra tenant, the same call also asks a directory that may simply not have answered, so the identical ACE classifies as `unresolved_foreign` and **refuses** under `standard` — `%LOCALAPPDATA%` included. If you are on such a machine, moving to another standard profile folder is not the fix: put the configuration and `state_root` under `%USERPROFILE%\.agentic-hil`, or set `windows_path_trust: permissive` in the configuration where a later reader can see it. The refusal states which case your machine is in, in its own remediation.

Two SID forms refuse on every machine, workgroup included, because they have no account name to be missing in the first place: a logon session SID `S-1-5-5-<high>-<low>`, which names one live logon session and rides in that session's access tokens, and an Entra principal `S-1-12-1-*`. A principal lookup that could not be performed at all, and an ACL that could not be read, still refuse too: those are ignorance rather than evidence, and `windows_path_trust: permissive` is the documented answer for an environment — a sandbox, a service account, a restricted CI runner — whose ACLs genuinely do not describe its trust boundary.

`agentic-hil doctor` reports what the check saw and accepted under `path_trust`: the active mode, one entry per finding, and who holds the rights. `path_trust.ok: true` means nothing was found; a populated `findings` list is a named degraded environment, not a failure.

On Windows the refusal names who holds the right, in `untrusted_principals`: the SID, its `principal_class`, plus the application package it belongs to when the SID resolves. A capability SID `S-1-15-3-<sub-authorities>` is resolved through the package SID `S-1-15-2-<the same sub-authorities>`, which Windows registers under `HKLM\SOFTWARE\Microsoft\SecurityManager\CapAuthz\ApplicationsEx` and in the AppContainer mappings under `HKEY_CLASSES_ROOT`. That turns "an unknown principal has rights here" into a decision you can actually take: use a permitted location, or have the operator remove that application's grant through the application itself.

If the environment genuinely has no meaningful ACLs, set `windows_path_trust: permissive` in the authoritative configuration. That is the documented, visible override; redirecting `%APPDATA%` or `%LOCALAPPDATA%` until the refusal stops is not, because nothing afterwards records which policy applied. `windows_path_trust: strict` restores the pre-0.8.0 rule, where an app-package identity and an unresolvable SID also refuse. Whichever you set, it reaches that configuration's own `state_root` and the directories derived from it: the directory holding the configuration is read before the key can be, and the MCP server executable and `~/.agentic-hil/device-locks` are shared beyond the project, so those three are always checked under `standard`.

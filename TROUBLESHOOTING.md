...
Every result on this page is one document, and `agentic-hil` renders it for a person unless something says otherwise: a refusal opens with `Refused: <error_type>`, followed by its summary, the details it carries and its numbered remediation, and `doctor` comes back as a section-by-section report rather than as a document. A refusal that ran something brings back what came back: the nested object under the key that ran it is rendered under that key, and a captured `stdout` or `stderr` is printed the way the tool wrote it, line for line, so the sentence that says why is on the screen without a second command. A pipe and a redirect are rendered the same way, because a wrapper that captures this command and then shows somebody what came back is what almost every caller is. `--json` prints the document itself, unchanged and field for field, and every subcommand takes it: that is the spelling for reading one field with `jq`, for pasting the whole thing into an issue, and for anything that parses this command at all.
...
```bash
python -m pip install --user --upgrade "agentic-hil>=0.21.5"
agentic-hil --version
...
```bash
uvx --from "agentic-hil>=0.21.5" agentic-hil --version                           # transient diagnostic only
uvx --from git+https://github.com/agentic-hil/agentic-hil@v0.21.5 agentic-hil --version # transient repository check
uv tool install --upgrade "agentic-hil>=0.21.5"                                  # persistent user-local install
```

`pipx run --spec "agentic-hil>=0.21.5" agentic-hil --version` is also only a transient diagnostic; use `pipx install "agentic-hil>=0.21.5"` for a persistent installation. If `agentic-hil` is installed but not found, add the user-level executable directory to `PATH` with `uv tool update-shell` or `pipx ensurepath` and open a fresh shell. If neither `uv` nor `pipx` exists, install `uv` user-locally first (`curl -LsSf https://astral.sh/uv/install.sh | sh`). Never use `sudo pip` or `pip install --break-system-packages`.
...
**`error: externally-managed-environment` (PEP 668), or a Python that has no pip.** The script prints `this Python is externally managed (PEP 668), so pip cannot own it; falling back to uv` and continues into the uv branch by itself, which is the correct resolution: `uv` installs into an environment of its own and needs no exception. A `python3` carrying no pip module at all, which is what Debian and Ubuntu servers and most minimal container images ship unless `python3-pip` is installed, takes that same branch before an install is attempted with it: step 2 says `python3 has no pip module, so pip cannot install with it; falling back to uv`, fetches `uv` when the machine has none, and installs with that. A pip that is there and then fails for some other reason is a real failure and still stops the run with `pip could not install`. If the uv fallback also fails, install `uv` or `pipx` by hand and run the one-liner again. Never reach for `pip install --break-system-packages`; neither script offers it, and it makes the distribution's own Python the thing that breaks next.

**Step 3 says the command does not resolve on `PATH`.** The package is installed and its console script landed in a directory your shell does not look in, so step 3 adds that directory: one line in each startup file your shell reads, each named in the transcript, or on Windows the directory in front of your own `Path` value, with the kind that value already had kept so nothing else in it is rewritten. Bash on Linux reads two, `~/.bashrc` in a new terminal window and its login file in the login shell an ssh session or a console starts, so the line goes in both; if a new terminal window finds the command and a login shell does not, the line is in `~/.bashrc` alone, which is where an earlier installer put it, and running the installer again adds it to the login file. Open a fresh shell and check with `agentic-hil --version`; on Linux and macOS a shell that is already open, the one you ran the installer from included, read its files before the line was there, and running the line step 3 prints in it does the same. A non-interactive shell, such as `ssh host 'agentic-hil doctor'`, a cron job or a CI step, may read none of those files, and cron and CI run a POSIX shell even when yours is fish: call the command there by the full path step 3 prints, or put its directory on that shell's `PATH` first. Run the installer with `--no-path` (`-NoPath` in PowerShell) to keep that edit for yourself: step 3 then prints the exact line and writes nothing, which is also what it does for a file or the value it cannot write. If step 3 printed no directory at all, section 1 above has the wider fix.
...
Fix: run `agentic-hil init`, review the file it writes outside the repository (every permission in it is granted except `allow_raw_debugger_commands` and `allow_mass_erase`, which are false so that flashing works), then run `agentic-hil doctor` again. Run `agentic-hil setup --agent <claude-code|codex|opencode>` instead when the agent on this machine is yours to register as well: it installs that agent's skill and registers the MCP server, then does this same project half. Those two are what the refusal itself names first at a terminal; the same refusal read as a document names `project_config_create`, which is the agent's route to the same file. Set `AGENTIC_HIL_CONFIG` only if an explicit absolute-path override is required. Use structured fields such as `field`, `rejected_fields`, `allowed_fields`, `allowed_values`, and `expected_type` to fix schema errors. A refusal that also carries `written_by_release` is not a file to correct: a newer Agentic HIL wrote those keys, and `agentic-hil upgrade` is the fix.
...
On that OpenOCD path the USB serial inventory is what enumerates the probes, and it is a real reading rather than an authoritative count: it reaches an ST-Link only through the virtual COM port a V2-1 or a V3 publishes, so a standalone ST-LINK/V2, or any probe with no VCP, can be attached and never appear in it. One visible ST-Link is bound anyway, because that is what lets `agentic-hil init` from the project root reach a working bench on a host with OpenOCD and nothing from ST, and the blind spot is named on the result rather than turned into a refusal: the discovery answer and the generated `debuggers.<name>` entry both carry `discovered_by: usb_serial_inventory`, `probe_inventory: incomplete` and the sentence saying which serial was bound and what that reading cannot see, and `agentic-hil init` and `project_config_create` repeat it in a step of their own. Where a probe the inventory cannot see is attached beside the visible one, name the intended board with `agentic-hil adopt-hardware --probe-id <serial>`; where an authoritative count is what matters, install STM32CubeProgrammer, whose own listing reads the serial off the probe itself. Two or more visible probes are `ambiguous_hardware` as on either enumeration, and a reading with no ST-Link in it at all is `probe_inventory_incomplete`: there is nothing to bind and nothing to conclude, so `agentic-hil init` writes an unbound placeholder and does not tell you to attach a bench that may be plugged in already.
...
* **`permission_change_in_open_run`** means something on this machine is holding the bench: a run, a COM or CAN session, another terminal. Permissions do not move under an active hold. `agentic-hil lease-status` names the holder; wait or ask for it to be closed, then repeat the command. `agentic-hil init --force` answers `config_write_in_open_run` under the same rule and for the same reason (a regeneration replaces every permission, baudrate and device binding those holds were taken under), and it refuses before reading the board, so nothing on the bench is touched to reach the answer.
...
The part was identified, the voltage and SWD frequency in the same transcript are normal, and an immediate second attempt programs and verifies in a couple of seconds. The pattern repeats every session, and the failures take about the same time as each other.

What it means: the device refused to erase the sectors this image covers. It is not a reset failure and not a probe failure. The measured case is a core still executing from flash when STM32CubeProgrammer connects in hot-plug mode: a hot-plug connect leaves the target running, so the firmware already on the board is still fetching instructions out of the memory the erase is clearing, and it interferes with it, which is why it hits the first flash after power-up and not the retry a second later. Halting the target first does not help here: STM32CubeProgrammer reboots the target on every invocation of the CLI, so a halt from an earlier call has not survived into the flash call. The other cause worth checking is protection over those sectors: read-out protection, write protection or PCROP.

What it says about the board: a refused erase does not prove the board was left untouched, so `flash_firmware` treats the flash as unconfirmed and opens a `debugger_result_unconfirmed` incident whichever reading applies. `erase_abort_point` is a diagnostic reading of the transcript, taken rather than from the exit code, with `evidence_line` naming the line it was read from. `erase_refused_effect_unconfirmed` means the refusal is there and no line reports a write completing, but the transcript does not establish that no sector was erased before the refusal, STM32CubeProgrammer's progress and download lines are not guaranteed across versions, quiet or piped output and abort paths, and `failed to erase memory` also covers a protection refusal that can strike after unprotected sectors in the same range have already been erased. `flash_change_underway` means a line says a phase had started, so flash is neither the old image nor the new one; `abort_point_unreadable` means the transcript does not place the failure at all. This is an ordinary `debugger_result_unconfirmed` incident that owes no gate. A bare `flash_firmware` call is its own implicit single-action run, so its incident stands down the moment the call ends: the result carries `incident_stood_down` and `quarantined: false`, and the bench is handed back automatically. A call made inside a declared run (`bench_run_start` … `bench_run_stop`) is different, the run owns the hold, so the failed result stays `quarantined: true` with no `incident_stood_down` and the declared run keeps the probe until `bench_run_stop`, which performs the run teardown and the same stand-down then. Either way the incident owes nobody a signature: `hardware_recover` over it answers `nothing_to_recover: true`, and the next flash is accepted without any recovery step. Section 13's `recover --confirm-safe-state` is for a quarantine that actually holds the bench (`resource_quarantined`, a broken audit), which this is not. What does not change is that the board holds an indeterminate image until a retry programs and verifies it, so a retry taken as proof that nothing was erased is the one reading to avoid.

Durable fix: set `debuggers.<name>.connect_mode: under_reset` on that probe, which makes the flash connect with STM32CubeProgrammer's `mode=UR` and holds the target in reset while the probe attaches, so the core never runs during the erase. It needs the probe's reset line wired to the target's NRST, which an on-board ST-Link on a Nucleo has. The key is read by `flash_firmware` alone (a probe stays hot plug because it is the least intrusive call and erases nothing), it is read only by `type: stlink` (OpenOCD takes the same effect from the scripts named by `interface_cfg` and `target_cfg`, and this server sets nothing of the kind for pyOCD, so both refuse the value at load instead of ignoring it), and a running server picks it up the way it picks up any other device description, through `project_config_reload_description` or a restart. Full description: [The Authoritative Configuration](docs/configuration.md#connecting-under-reset).

If the refusal repeats even under reset, read the option bytes yourself (`STM32_Programmer_CLI -c port=SWD -ob displ`) and look at the protection over the sectors the image covers, rather than at the reset line.

Workaround until the configuration change reaches the bench: flash again immediately. There is nothing to recover first and the retry is simply accepted, a bare call's incident has already stood down when its call ended, and a retry made inside a declared run is one the bench lets through while the run keeps the hold until `bench_run_stop`. The retry is what has always worked on this pattern, and it works for the same reason the fix does, one attempt later. The refused erase leaves the flash unconfirmed rather than provably untouched, so read the retry as writing over an unknown image, not as a free retry on a board you have proved is clean. Until then it belongs in the plan rather than in somebody's memory.
...
First, check which half you actually need. On `type: stlink` and on `type: pyocd` the three symbol tools work without a session: `debug_symbol_info` says where an allowed symbol is and how large it is (it opens no probe at all, resolving out of the ELF a confirmed `flash_firmware` put on the board), and `debug_symbol_value` and `debug_dump_symbol_ihex` read those bytes off the target with the backend's own memory read, STM32CubeProgrammer's `-r` or pyOCD's `savemem`. If the step wanted a value out of RAM, a coverage buffer or a counter, that is the tool, and nothing about the bench has to change. Flash the ELF through `flash_firmware` first, keep the symbol in `debug.allowed_symbols`, and have `debug.gdb_executable` name a GDB that reads that image, because the resolution both backends run first is a GDB against the file.

Those reads connect with the least intrusive connect each tool documents, because a read that reset the target would destroy the RAM it was asked for: `mode=HOTPLUG` on STM32CubeProgrammer, which attaches to the running core rather than resetting it, and `--connect attach` on pyOCD, which is the one of its four connect modes (halt, pre-reset, under-reset, attach) that its own documentation describes as connecting to a running target without halting cores. Neither reads `debuggers.<name>.connect_mode`: on `type: stlink` that key belongs to the flash, and on `type: pyocd` it reaches nothing at all, which is why `under_reset` is refused there when the file is loaded.

What is genuinely refused is the session half: breakpoints, continue, halt, stop reasons, and the session lifecycle around them. Those are GDB operations, and neither STM32CubeProgrammer nor pyOCD is a GDB server this service drives as one.

The same split decides what a test plan may carry, and a plan is answered before its first step rather than by a step that fails halfway: `dump_memory` and `read_symbol` need no `debug_start` before them on a backend that serves those reads with no session, and a plan carrying a step this bench serves neither way is refused at preflight, naming the backend, the step and its number, which reads that backend does serve, and the same change described next.
...
`com_port_not_bound` is the separate case: the port is configured, and its `device` is empty because `agentic-hil init` wrote the project profile's ports without a bench attached. The plan or the call is right and the file is not filled in. The refusal names the exact command for this bench under `next_step`: `agentic-hil adopt-hardware` writes the device together with the adapter's serial number and USB ids. Because adoption selects a debugger before it reads the board, that command names one with `--debugger <name>` where the bench declares several, and on a bench that declares no debugger at all it sends you to fill `com_ports.<name>.device` by hand instead, since adoption cannot run there. Do not delete the entry; that turns a precise refusal back into `com_port_not_configured`.
...
`device_busy` is not a fault. Another run holds that physical device for its whole duration, which is the exclusivity that replaced the read permission. The refusal carries `holder` (pid, host, frontend, and the run label when there is one) and `held_since`; nothing was touched. Wait for that run, or ask its owner to finish. If waiting is right, ask for it explicitly and bounded: `agentic-hil test-reactor --wait-s <seconds>`. If the holder is a test run you want to end early, end it in an orderly way rather than by killing it: `agentic-hil test-reactor-status` lists the runs this bench knows, and `agentic-hil test-reactor-stop --run <handle>` asks one to finish the step it is in, close its devices and write its report. Never delete a lock file under `~/.agentic-hil/device-locks`: the hold belongs to a live process, and removing it lets two runs drive one board.
...
What decides it is the backend's claim, not that the call was a read. A read names no abort point when it was killed at its deadline (`timeout`), or when a toolchain exited without a complete confirmation (`target_state_unconfirmed`). The result names which markers were expected and which arrived, and a partial set still names no safe abort point. Such a read quarantines as `debugger_readonly_target_state_unconfirmed`: an SWD attach halts the core, a killed process never ran the `shutdown` in its own command string, and a re-read that finds the probe again does not resume a halted core. `target_state_unconfirmed` is its own `error_type` for that reason: `target_not_detected` beside it is the backend's positive report that it reached the adapter and nothing answered, which is why that one refuses and this one does not. That reason is settled by a verified reset-into-halt or by an operator, never by the read-only predicate. A PCAN `CanInitializationError` likewise quarantines, because python-can raises it from `SetValue` calls that run after `PCANBasic.Initialize` has already put the channel on the bus. Two PCAN failures do prove the adapter never joined and refuse instead: `can_adapter_library_missing`, when the PCAN-Basic API is not installed and so no driver object existed, and `can_channel_not_available`, when `Initialize` answered that the handle is invalid or the driver enumerates channels and this is not one of them.
...
- `auto_recoverable: true` means the running owner clears this incident itself on the next hardware tool call, then runs it. It reaps this owner's leftover debugger processes, verifies the safe state, records the attestation as `recovery: machine_attested`, and proceeds. No operator step. If verification fails, the incident stands down at the end of that call and the call proceeds, answering for itself with a `recovery` block that says `outcome: failed` and which action failed (`failed_action`). What stands is a recovery whose own report could not be written: that is `machine_recovery_audit_broken`, the call returns `resource_quarantined` with `auto_recovery_attempted: true`, and the quarantine is now the operator's. A `resource_quarantined` refusal without that field attempted nothing, because the incident was never machine-recoverable. After `recovery.max_attempts` failures for the same incident it stops retrying.
...
|---|---|---|
| `ok: true`, `upgraded_on_disk: true` | `previous_version` and `version` differ; the summary names both | Restart what `restart_required` says needs it. `restart_required_by` names the processes still running the previous release, by pid, the directory each was started in and when it started, and the summary names the agent hosts that have this server registered |
| `ok: true`, `already_current: true` | Nothing newer to install, both version fields are the same number, and `newest_release` is the index's own answer, asked before the claim was made. The summary says whether this build is level with that release or a build ahead of it | Nothing, unless `restart_required` is true. It is when a server started out of this installation is still up: nothing was replaced here, so that process answers with whatever was on disk when it started, and only its restart makes that this release. A host that publishes no process table carries no `restart_required` at all and says so in the summary |
| `ok: true`, no `already_current` and no `newest_release` | The index did not answer, so which release is the newest is not something this result can say. Nothing was installed | Nothing, unless a newer release was expected. Then check this machine's route to the index and run it again |
| `ok: true`, no `already_current`, with `newest_release` | The index publishes something newer and this installation stayed where it is, with nothing recorded that explains it: a private index, a release this interpreter cannot take, or a recorded floor, which moves with the index and holds nothing back. `held_back_by` appears here only on an installation pinned to the release it is already on whose receipt also records an option, which is the one thing that check has not cleared | Nothing is wrong and nothing was withheld, so this exits 0. Check the route to the index, or run the line in `reinstall_command` where the result carries one |
...
| `ok: false`, `error_type: upgrade_blocked_by_recorded_option` | The manager had nothing to install while the index publishes something newer, and `held_back_by` names what this installation's own receipt records and what is demonstrably keeping it below that release: an `exclude-newer`, or an exact `==` requirement the manager did not name in words. A `>=` or `~=` floor is never one of them, because a floor resolves to whatever the index offers above it. No `uv tool` command clears either | Run the line in `reinstall_command`. It records the installation again without the option and keeps the extras, the `--with` packages and the interpreter it has, and names under `with_packages_not_replayed` anything the receipt records that a `--with` cannot spell |
| `ok: false`, `error_type: upgrade_failed`, `installation_intact: true` | The manager failed and the installation it was replacing still works; `version` is what it still runs | Fix what the manager reported and run it again, or run the install line from section 1: it repairs in place. Nothing was lost. `restart_required` reads as on every other outcome: this run replaced nothing, so `restart_required_by` names the servers started out of this installation that may predate its last upgrade |
| `ok: false`, `error_type: installation_changed_after_failed_upgrade` | The manager failed but had already replaced files: the disk now loads a different version (`version`) from the one still running (`previous_version`) | Run the line in `reinstall_command`, with the host stopped. Do not restart onto the half-changed tree. The servers still running out of the installation are named in the summary as a notice, never as a restart request, and `restart_required` stays false |
| `ok: false`, `error_type: installation_broken` | The manager stopped part way and the package did not survive it: `agentic-hil` still starts and dies with a `ModuleNotFoundError` | Run the line in `reinstall_command`, with the host stopped. It carries the extras and the scope this installation had. The install line from section 1 is the same repair without the extras spelled by hand. The running servers are named the same way as on the half-changed tree |

Only the pinned case, the recorded-option case and the two failures are refusals, and only they exit non-zero. A machine that is already at the newest release has nothing wrong with it, and neither has one whose recorded requirement is a `>=` or `~=` floor, so a provisioning script may run `agentic-hil upgrade` unconditionally on a current installation and on a floored one. What exits non-zero on an installation that is not behind anything is an exact `==` requirement or a recorded option that is holding an older release in place, which is a decision somebody made and has to unmake. Read `restart_required` rather than assuming one: it is true when a server started out of this installation is still running, false when the process table was read and held none, and absent, with a sentence saying so, on a host that could not read its process table at all. On a run that moved the version, `agentic-hil upgrade` also raises it for an agent host that has this server registered; a run that replaced nothing never looks at the registrations, because there is nothing new for a restart to pick up.
...
**A failed upgrade says whether it left an installation.** After the manager stops, `agentic-hil upgrade` runs the same import the console script runs, through the same interpreter, and the version it loads is what tells the three failure rows above apart, measured rather than assumed: the same number still running is intact and unreplaced, a *different* one is a tree the failed run changed on disk before it stopped, and nothing loadable is a broken installation. The half-changed case names both numbers and refuses `restart_required`, because the version a failed run left on disk is not one to adopt by restarting onto it; `reinstall_command` restores a whole known installation instead, and the processes still running out of the installation are named as a notice only. The intact failure carries them the way the upgraded outcome does, read before the manager ran: `restart_required: true` with `restart_required_by` where any is running, false where the table was read and held none, and absent, with the sentence, where it could not be read. A `/proc` that lists no entry for the process reading it, which is what a chroot and a minimal container image carry, is a table that could not be read and not an empty one. On a per-user `pip` installation the upgrade also passes `--user`, which keeps the replacement in the same directory the old copy was in; without it pip uninstalls from the per-user site and installs into the interpreter's default site, and a failure between those two steps is what leaves a console script with no package under it.

**What the manager said comes back with it.** Every one of those failures carries `install`, with the code the manager exited with and the output it wrote, and the rendering prints it under `Details`, indented and line for line, so "fix what the manager reported" is something to read on the screen rather than a reason to run the command again with `--json`. A `verification` beside it is the import that decided which of the three rows this is. Enormous captures are cut in the middle and say how many lines they cut, so the first and the last of them, which is where a manager puts the reason, are both there.

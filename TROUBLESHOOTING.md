...
**Step 3 says the command does not resolve on `PATH`.** The package is installed and its console script landed in a directory your shell does not look in, so step 3 adds that directory: one line in the one shell profile your shell reads, named in the transcript, or on Windows the directory in front of your own `Path` value, with the kind that value already had kept so nothing else in it is rewritten. Open a fresh shell and check with `agentic-hil --version`. Run the installer with `--no-path` (`-NoPath` in PowerShell) to keep that edit for yourself: step 3 then prints the exact line and writes nothing, which is also what it does when the file or the value cannot be written. If step 3 printed no directory at all, section 1 above has the wider fix.

...
Two things it will not do. `debuggers.<name>.type` decides which program drives the board, so a discovered ST-Link toolchain is not written into an entry whose type is `openocd`; the result says so and names the entry. And a `com_ports` entry it creates arrives with every permission `false` — writing to a port stays yours to grant.

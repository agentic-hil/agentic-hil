...
Every result on this page is one document, and `agentic-hil` renders it for a person unless something says otherwise: a refusal opens with `Refused: <error_type>`, followed by its summary and its numbered remediation, and `doctor` comes back as a section-by-section report rather than as a document. A pipe and a redirect are rendered the same way, because a wrapper that captures this command and then shows somebody what came back is what almost every caller is. `--json` prints the document itself, unchanged and field for field, and every subcommand takes it: that is the spelling for reading one field with `jq`, for pasting the whole thing into an issue, and for anything that parses this command at all.
...
```bash
python -m pip install --user --upgrade "agentic-hil>=0.17.0"
agentic-hil --version

...
`%APPDATA%` and `%LOCALAPPDATA%` are not this case from 0.8.0 on. They are standard Windows working folders and they pass, including on a profile where an installed packaged application holds an app-capability ACE on `AppData`: the grant is real, but it belongs to an AppContainer identity holding a subset of your own rights, and your ordinary unpackaged processes can rewrite the file anyway. What is still refused is a **nameable account other than you, SYSTEM or Administrators** holding write, delete or WRITE_DAC on the path or an ancestor. A SID the security authority answered about, saying nothing here maps to it, is reported instead; a stock `%LOCALAPPDATA%` carries one. A principal lookup that could not be performed at all, and an ACL that could not be read, still refuse: those are ignorance rather than evidence, and `windows_path_trust: permissive` is the documented answer for an environment — a sandbox, a service account, a restricted CI runner — whose ACLs genuinely do not describe its trust boundary.

...
`%APPDATA%` and `%LOCALAPPDATA%` are not this case from 0.8.0 on. They are standard Windows working folders and they pass, including on a profile where an installed packaged application holds an app-capability ACE on `AppData`: the grant is real, but it belongs to an AppContainer identity holding a subset of your own rights, and your ordinary unpackaged processes can rewrite the file anyway. What is still refused is a **nameable account other than you, SYSTEM or Administrators** holding write, delete or WRITE_DAC on the path or an ancestor. A SID that resolves to nothing, and an ACL that could not be read, are reported instead — the latter is what used to lock out sandboxes, service accounts and CI runners.

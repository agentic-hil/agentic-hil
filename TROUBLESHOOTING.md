...
Workaround until the configuration change reaches the bench: recover the incident (section 13), then flash again immediately. The retry is what has always worked on this pattern, and it works for the same reason the fix does, one attempt later; because the refused erase leaves the flash unconfirmed rather than provably untouched, clearing the incident is the step before it. Until then it belongs in the plan rather than in somebody's memory.

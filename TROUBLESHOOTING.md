...
What it says about the board: whether the image that was already there survived depends on where the failure hit, and `erase_abort_point` is that reading, taken from the transcript rather than from the exit code, with `evidence_line` naming the line it was taken from. `erase_refused_before_flash_changed` means no line reports an erase or a download having started, so the board still holds the image it held: the call is a refusal, no incident is opened, and there is nothing to recover before the retry. `flash_change_underway` means a line says a phase had started, so flash is neither the old image nor the new one and the `debugger_result_unconfirmed` incident stands; `abort_point_unreadable` means the transcript does not settle it, and the incident stands for that reason. In both of those, section 13 is the way out and a retry on the assumption that nothing was erased is not.
...
Workaround until the configuration change reaches the bench: flash again immediately. The retry is what has always worked on this pattern, and it works for the same reason the fix does, one attempt later; until then it belongs in the plan rather than in somebody's memory.

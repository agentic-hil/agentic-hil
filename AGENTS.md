...
Agentic HIL is the hardware gate. The project-local `.agentic-hil/config.yaml` is an untrusted project request; the host-managed `AGENTIC_HIL_POLICY` outside the workspace is the authorization ceiling. Agentic HIL adapters are the reference hardware.
...
If an Agentic HIL tool returns `permission_denied`, stop. Do not edit or create the trusted policy, its environment variable, or host MCP registration; those are human/operator-controlled.

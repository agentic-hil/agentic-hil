...
`config_stale: true` on any result means the same thing arrived the other way: the authoritative file is no longer the one this server parsed, and every answer — the backend it names, the devices it knows, the permissions it enforces — still comes from the version loaded at startup. The `config_status` block says which file (`path`) and which of three states it is in, and the remedy is not the same for all three: `changed` — `loaded_digest` and `current_digest` differ, and a restart loads the file that exists now; `missing` — the file is gone and `current_digest` is `null`, so it has to be restored *before* a restart can load anything, and `project_config_describe` meanwhile answers out of the loaded policy (`permissions_in_force`, `document_source: loaded_policy`); `unreadable` — it is there and will not open, `current_digest` is `null` and `backend_error` names what the read failed on, so it has to be made readable and valid before a restart. `unknown` is not stale: nothing was compared, so nothing is claimed and `reload_required` is `false`. Nothing reloads while the server runs, so ask for a restart **once**, report what the answers currently say, and do not try to make the server pick the file up by editing it again or by calling a configuration tool. `debugger_info` and `project_config_describe` carry the block whether or not anything has moved, including when they refuse; `agentic-hil doctor` reads the file fresh every time, so a disagreement between it and `debugger_info` is this and nothing else. A permission revoked since startup is already in force; one added since is not.
...
| `agentic-hil://reference/platform-paths` | which locations pass the path trust check and what it does and does not defend, how a Windows refusal names the principal holding the right and what its `principal_class` means, what `windows_path_trust: standard \| strict \| permissive` changes, why `AGENTIC_HIL_CONFIG` and a chosen `state_root` are the supported answer to a refused location |

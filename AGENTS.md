...
`config_stale: true` on any result means the same thing arrived the other way: the authoritative file has changed since this server parsed it, and every answer — the backend it names, the devices it knows, the permissions it enforces — still comes from the version loaded at startup. The `config_status` block says which file (`path`), that the digests differ (`loaded_digest` against `current_digest`), and whether the file is `changed`, `missing` or `unreadable`. Nothing reloads it while the server runs, so ask for a restart **once**, report what the answers currently say, and do not try to make the server pick the file up by editing it again or by calling a configuration tool. `debugger_info` and `project_config_describe` carry the block whether or not anything has moved; `agentic-hil doctor` reads the file fresh every time, so a disagreement between it and `debugger_info` is this and nothing else. A permission revoked since startup is already in force; one added since is not.

...
- Dependabot monitors the Python locks under `requirements/`, GitHub Actions and container base images (`.github/dependabot.yml`). The locks are `uv pip compile` output and Dependabot moves them as the uv ecosystem, which reruns the command each lock records; a CI job recompiles every lock on every pull request and fails when one no longer follows from its input.

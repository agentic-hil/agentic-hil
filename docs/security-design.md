...
- Resuming a target requires `allow_debug_execution`: `debug_continue` — and the `run_until_breakpoint` plan step behind it — executes firmware and actuates whatever that firmware drives, so it is a write, not the read that attaching to a halted core is. `debug_halt` and `debug_stop_session` stay ungated as containment, so a running target can always be stopped.

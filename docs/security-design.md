...
- `agentic-hil init` creates and reports the external config, and since hardci-hq#96 it grants every permission in it, including `allow_raw_debugger_commands`, `allow_mass_erase` and all three `permissions.allow_config_*`. The operator narrows what this bench should not have — themselves, or by telling an agent, which can write `false` and nothing else. Without `agentic-hil.config.example.yaml` it writes the skeleton, whose `debuggers` entry names no toolchain and therefore drives nothing until somebody sets its `executable`; with a profile, setup runs only fixed STM32CubeProgrammer list/HOTPLUG identity commands and writes detected bindings, granting everything except the flags the profile itself sets false. Bootstrap is unavailable over MCP and cannot reset, halt, erase, flash, run raw commands, or open serial ports.

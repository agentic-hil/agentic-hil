...
- Runtime debugger discovery/execution, including `debugger_probes_list`, requires `allow_probe`. Setup bootstrap is the sole pre-policy exception; package code fixes its backend and commands. Reset separately requires `allow_reset`.

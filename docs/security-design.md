...
- A quarantine names its `cleanup_reasons`, and only a closed set of host-side faults (`RETRYABLE_CLEANUP_REASONS`) may be cleared without an operator. That machine recovery requires all of: an incident this live owner raised and still holds every resource of, one single retryable reason, intact audit state, no other active lease, `allow_probe`, a successful reap of this owner's debugger processes, a read-only `probe_target` re-read that detects the target, and a durable `recovery: machine_attested` report written *before* the state transition. It never drives the target, so it cannot change the physical state it is attesting to. An unconfirmed `flash_firmware`/`reset_target`/session start, any broken audit, and any incident adopted from a dead owner stay operator-gated.

...
That USB serial inventory is not an authoritative count of what is attached: it reaches an ST-Link only through the virtual COM port a V2-1 or a V3 publishes, so a standalone ST-LINK/V2, or any probe with no VCP, can be plugged in and never appear (`complete: false`), which means an empty reading is not proof no probe is connected and a sole visible one is not proof it is the only one. So this path never binds a probe off the count alone. `agentic-hil init` writes the `type: openocd` entry with its scripts but leaves `probe_id` unbound (the placeholder), and `project_config_create` over MCP refuses `probe_inventory_incomplete` rather than committing a board a later flash or reset would silently trust. Binding the visible serial takes an explicit confirmation: `agentic-hil adopt-hardware --probe-id <serial>` (the serials the host is showing are under `hardware_discovery.probes` and from `agentic-hil debugger-probes`), or STM32CubeProgrammer installed for an authoritative count, under which the generation binds the sole probe on its own and bare `agentic-hil adopt-hardware` fills it in.
...
It reads the attached probe and fills in what is still unset: the probe serial, the backend's executable, the GDB this host answers with (`debug.gdb_executable`), the detected controller, and the COM device the probe itself exposes. Nothing else: it supplies no value of its own, and its arguments only select (`--probe-id` names which probe this is about, `--debugger` and `--com-port` name which configured entries to fill). On an OpenOCD-only host `--probe-id <serial>` is required even for a lone board, because that USB serial inventory is not a complete count and bare adoption refuses `probe_inventory_incomplete` rather than binding off it; where STM32CubeProgrammer supplies an authoritative count, or among several attached boards it disambiguates, the same flag selects. A key that already holds a value nobody generated is reported as a disagreement and left alone, so a bench somebody set up is never repointed because something else happens to be plugged in; and an entry that already names a probe with a *different* probe attached is refused whole rather than partly carried, because the identity keys describe one board between them. A `com_ports` entry it creates arrives with every permission `false`, written by the server: a generation grants, a write only ever takes away, and adding an entry is a write. `agentic-hil init --force` is what opens a new entry.

...
`init` reads the attached bench while it does that, and it needs one debug toolchain on the host, not a particular one: with STM32CubeProgrammer installed it enumerates the ST-Link through that CLI, and otherwise out of this host's USB serial inventory, driving it with the `openocd` on `PATH`. That USB serial inventory is not an authoritative count (it cannot see a VCP-less ST-LINK/V2), so on the OpenOCD-only path `init` binds no probe off it: it writes the `type: openocd` entry with `probe_id` unbound, exactly the placeholder it writes with nothing plugged in, and `project_config_create` over MCP refuses `probe_inventory_incomplete` rather than committing a board off an incomplete reading. Its summary and next steps say which binaries were looked for, where each resolved and which ST-Link serials the host is showing, so a run that wrote placeholders says why. Where the workspace ships an `agentic-hil.config.example.yaml`, a placeholder still carries that profile's target and port names. Binding the visible serial takes `agentic-hil adopt-hardware --probe-id <serial>` on that path (a bare `agentic-hil adopt-hardware` refuses the same incomplete count); with STM32CubeProgrammer installed its authoritative count lets the sole board bind and bare `agentic-hil adopt-hardware` fills the devices in.

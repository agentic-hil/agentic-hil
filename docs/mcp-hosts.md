...
## Generated by `agent-install` (recommended for Claude Code / Codex / opencode)
...
| changes something reversible (`destructiveHint: false`) | `probe_target`, `artifact_upload`, `reset_target`, `debug_stop_session`, `debug_set_breakpoint`, `debug_clear_breakpoints`, `debug_continue`, `debug_halt`, `com_session_stop`, `can_session_stop`, `bench_run_start`, `bench_run_stop`, `hardware_recover`, `project_config_adopt_hardware` |

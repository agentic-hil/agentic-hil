...
The command exits `0` when every check passed and `1` when any failed, so it can
be a step in a script.
...
| `debugger-probes` | probe discovery answers, and says how far its count reaches, and exits zero for saying so |
...
| `check-plan` | every plan your project ships loads through the reactor's loader |
...
- **Your configuration is not read and not replaced.** The battery redirects the
  ...
  your project already has stays where it is, unread.
...
- **Your project directory is used as it stands.** The product writes its reports
  and logs under `.agentic-hil` there, as it does for any run, and the battery
  writes its own plans as `bench-battery-*.yaml` beside them. The permission
  check revokes one permission in its own configuration and grants it back
...
identity was there and how many, never what it said. A report is meant to be
sendable, and the identifiers of somebody's bench are not part of what it has to
say.
